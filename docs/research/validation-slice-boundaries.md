...
2. **LLM 独立初审**：以独立 reviewer prompt 和调用读取原始证据包，质疑证据充分性、搜索意图、动作合理性及证据冲突，并输出“建议接受”“建议拒绝”或“升级人工判断”。MVP 可以复用同一基础模型，但不能复用生成调用的上下文来完成自我认证。
3. **SEO 责任人终审**：由用户或其指定的单一 SEO 负责人作出“接受”“拒绝”或“证据不足”的最终判断并记录理由。
